# File taken from
# https://github.com/svalleco/3Dgan/blob/Anglegan/keras/analysis/utils/GANutils.py
...
def sortEnergy(data, ecal, energies, ang=1):
    ...
    for energy in energies:
        if energy == 0:
            ...
            var["indexes" + str(energy)] = np.where(
                (data[1] > (energy - tolerance) / 100.) & (data[1] < (energy + tolerance) / 100.))
...
def metric(var, energies, m, angtype='mtheta', x=25, y=25, z=25, ang=1):
    ...
    for energy in energies:
        ...
        var["pos_error" + str(energy)] = (np.absolute(var["posx_error" + str(energy)]) + np.absolute(
            var["posy_error" + str(energy)]) + np.absolute(var["posz_error" + str(energy)])) / 3
        ...
        sumxact, sumyact, sumzact = np.mean(var["sumsx_act" + str(energy)], axis=0), np.mean(
            var["sumsy_act" + str(energy)], axis=0), np.mean(var["sumsz_act" + str(energy)], axis=0)
        sumxgan, sumygan, sumzgan = np.mean(var["sumsx_gan" + str(energy)], axis=0), np.mean(
            var["sumsy_gan" + str(energy)], axis=0), np.mean(var["sumsz_gan" + str(energy)], axis=0)
        ...
        var["eprofile_total" + str(energy)] = (var["eprofilex_total" + str(energy)] + var[
            "eprofiley_total" + str(energy)] + var["eprofilez_total" + str(energy)]) / 3
        metrice += var["eprofile_total" + str(energy)]
        if ang:
            var["angle_error" + str(energy)] = np.mean(np.absolute(
                (var[angtype + "_act" + str(energy)] - var[angtype + "_gan" + str(energy)]) / var[
                    angtype + "_act" + str(energy)]))
...
def OptAnalysisShort(var, generated_images, energies, ang=1):
    ...
    for energy in energies:
        ...
        var["sumsx_act" + str(energy)], var["sumsy_act" + str(energy)], var["sumsz_act" + str(energy)] = get_sums(
            var["events_act" + str(energy)])
        var["sumsx_gan" + str(energy)], var["sumsy_gan" + str(energy)], var["sumsz_gan" + str(energy)] = get_sums(
            var["events_gan" + str(energy)])
        var["momentX_act" + str(energy)], var["momentY_act" + str(energy)], var[
            "momentZ_act" + str(energy)] = get_moments(var["sumsx_act" + str(energy)], var["sumsy_act" + str(energy)],
                                                       var["sumsz_act" + str(energy)], var["ecal_act" + str(energy)], m,
                                                       x=x, y=y, z=z)
        var["momentX_gan" + str(energy)], var["momentY_gan" + str(energy)], var[
            "momentZ_gan" + str(energy)] = get_moments(var["sumsx_gan" + str(energy)], var["sumsy_gan" + str(energy)],
                                                       var["sumsz_gan" + str(energy)], var["ecal_gan" + str(energy)], m,
                                                       x=x, y=y, z=z)
...
def OptAnalysisAngle(var, g, energies, ascale=None, xscale=None, yscale=100, xpower=None, latent=256, concat=1):
    ...
    for energy in energies:
        ...
        var["events_gan" + str(energy)] = generate(g, num,
                                                   [var["energy" + str(energy)], (var["angle_act" + str(energy)])],
                                                   latent, concat)
...
def get_sorted_angle(datafiles, energies, flag=False, num_events1=5000, num_events2=2000, tolerance1=5, tolerance2=0.5,
                     Data=GetAngleData, angtype='theta', thresh=0.0, offset=0.0):
    srt = {}
    for index, datafile in enumerate(datafiles):
        ...
        for energy in energies:
            if index == 0:
                if energy == 0:
                    srt["events_act" + str(
                        energy)] = X  # More events in random bin
                    srt["energy" + str(energy)] = Y
                    ...
                    if srt["events_act" + str(energy)].shape[0] > num_events1:
                        ...
                        print('For {} energy {} events were found in first file'.format(energy, srt[
                            "events_act" + str(energy)].shape[0]))
                        flag = False
                else:
...
def perform_calculations_angle(g, d, gweights, dweights, energies, angles, datapath, sortdir, gendirs, discdirs,
                               num_data, num_events, m, xscales, xpowers, angscales, dscale, flags, latent,
                               particle='Ele', Data=GetAngleData, events_per_file=5000, angtype='theta', thresh=1e-6,
                               offset=0.0, angloss=1, addloss=0, concat=1, pre=preproc, post=postproc, tolerance2=0.1,
                               num_events1=10000):
    ...
    # If reading from unsorted data. The data will be read and sorted in bins
        ...
        Filesused = int(math.ceil(
            num_data / events_per_file))  # num_data is number of events to be used from unsorted data/ events in each file
        Trainfiles, Testfiles = DivideFiles(datapath, datasetnames=["ECAL"],
                                            Particles=[particle])  # get test and train files
        ...
        start = time.time()
        var = get_sorted_angle(data_files, energies, True, num_events1, num_events2, Data=Data, angtype=angtype,
                               thresh=thresh * dscale, offset=offset)  # returning a dict with sorted data.
    ...
    for energy in energies:
        ...
        var["max_pos_act" + str(energy)] = get_max(
            var["events_act" + str(energy)])  # get position of maximum deposition
        var["sumsx_act" + str(energy)], var["sumsy_act" + str(energy)], var["sumsz_act" + str(energy)] = get_sums(
            var["events_act" + str(energy)])  # get sums along different axis
        var["momentX_act" + str(energy)], var["momentY_act" + str(energy)], var[
            "momentZ_act" + str(energy)] = get_moments(var["sumsx_act" + str(energy)],
                                                       var["sumsy_act" + str(energy)], var["sumsz_act" + str(energy)],
                                                       ecal, m, x=x, y=y, z=z)  # calculate moments
        for index, a in enumerate(angles):
            indexes = np.where(((var["angle" + str(energy)]) > np.radians(a) - tolerance2) & (
                    (var["angle" + str(energy)]) < np.radians(
                a) + tolerance2))  # all events with angle within a bin
            ...
            print('{} for angle bin {} total events were {}'.format(index, a, var[
                "events_act" + str(energy) + "ang_" + str(a)].shape[0]))
    ...
    for energy in energies:
        print(
            "{}\t{}\t{:.4f}\t\t{}\t\t\t{:.2f}\t\t{:.4f}\t\t{:.4f}\t\t{:.4f}".format(energy, var["index" + str(energy)],
                                                                                    np.amax(var["events_act" + str(
                                                                                        energy)]), np.mean(
                    var["max_pos_act" + str(energy)], axis=0), np.mean(var["events_act" + str(energy)]), np.mean(
                    var["momentX_act" + str(energy)][:, 1]), np.mean(var["momentY_act" + str(energy)][:, 1]), np.mean(
                    var["momentZ_act" + str(energy)][:, 1])))
    ...
    for energy in energies:
        ...
        for gen_weights, disc_weights, scale, power, ascale, i in zip(gweights, dweights, xscales, xpowers, angscales,
                                                                      np.arange(len(gweights))):
            ...
            if read_gen:
                ...
                start = time.time()
                var["events_gan" + str(energy)]['n_' + str(i)] = generate(g, var["index" + str(energy)],
                                                                          [var["energy" + str(energy)] / 100,
                                                                           (var["angle" + str(energy)]) * ascale],
                                                                          latent, concat)
                var["events_gan" + str(energy)]['n_' + str(i)] = post(var["events_gan" + str(energy)]['n_' + str(i)],
                                                                      scale, power)
                var["events_gan" + str(energy)]['n_' + str(i)][
                    var["events_gan" + str(energy)]['n_' + str(i)] < thresh * dscale] = 0

                if save_gen:
                    save_generated(var["events_gan" + str(energy)]['n_' + str(i)],
                                   [var["energy" + str(energy)], var["angle" + str(energy)]], energy, gendir)
                var["events_gan" + str(energy)]['n_' + str(i)] = pre(var["events_gan" + str(energy)]['n_' + str(i)],
                                                                     scale, power)
            ...
            if read_disc:
                ...
                print("Discriminator took {} seconds for {} data and generated events".format(disc_time, var[
                    "index" + str(energy)]))

                if save_disc:
                    discout = {}
                    for key in var:
                        if key in ["isreal_act" + str(energy), "aux_act" + str(energy), "isreal_gan" + str(energy),
                                   "aux_gan" + str(energy), "ecal_act" + str(energy), "ecal_gan" + str(energy),
                                   "angle2_act" + str(energy), "angle2_gan" + str(energy), "angle_act" + str(energy),
                                   "angle_gan" + str(energy), "addloss_act" + str(energy), "addloss_gan" + str(energy)]:
            ...
            var["isreal_act" + str(energy)]['n_' + str(i)], var["aux_act" + str(energy)]['n_' + str(i)], \
            var["angle_act" + str(energy)]['n_' + str(i)], var["ecal_act" + str(energy)]['n_' + str(i)] = np.squeeze(
                var["isreal_act" + str(energy)]['n_' + str(i)]), np.squeeze(
                var["aux_act" + str(energy)]['n_' + str(i)]), np.squeeze(
                (var["angle_act" + str(energy)]['n_' + str(i)])) / ascale, np.squeeze(
                var["ecal_act" + str(energy)]['n_' + str(i)] / (dscale * scale))
            var["isreal_gan" + str(energy)]['n_' + str(i)], var["aux_gan" + str(energy)]['n_' + str(i)], \
            var["angle_gan" + str(energy)]['n_' + str(i)], var["ecal_gan" + str(energy)]['n_' + str(i)] = np.squeeze(
                var["isreal_gan" + str(energy)]['n_' + str(i)]), np.squeeze(
                var["aux_gan" + str(energy)]['n_' + str(i)]), np.squeeze(
                var["angle_gan" + str(energy)]['n_' + str(i)]) / ascale, np.squeeze(
                var["ecal_gan" + str(energy)]['n_' + str(i)] / (dscale * scale))
            if angloss == 2:
                var["angle2_act" + str(energy)]['n_' + str(i)] = np.squeeze(
                    (var["angle2_act" + str(energy)]['n_' + str(i)])) / ascale
                var["angle2_gan" + str(energy)]['n_' + str(i)] = np.squeeze(
                    (var["angle2_gan" + str(energy)]['n_' + str(i)])) / ascale
            if addloss:
                var["addloss_act" + str(energy)]['n_' + str(i)] = np.squeeze(
                    (var["addloss_act" + str(energy)]['n_' + str(i)]))
                var["addloss_gan" + str(energy)]['n_' + str(i)] = np.squeeze(
                    (var["addloss_gan" + str(energy)]['n_' + str(i)]))

            var["max_pos_gan" + str(energy)]['n_' + str(i)] = get_max(var["events_gan" + str(energy)]['n_' + str(i)])
            var["sumsx_gan" + str(energy)]['n_' + str(i)], var["sumsy_gan" + str(energy)]['n_' + str(i)], \
            var["sumsz_gan" + str(energy)]['n_' + str(i)] = get_sums(var["events_gan" + str(energy)]['n_' + str(i)])
            var["momentX_gan" + str(energy)]['n_' + str(i)], var["momentY_gan" + str(energy)]['n_' + str(i)], \
            var["momentZ_gan" + str(energy)]['n_' + str(i)] = get_moments(var["sumsx_gan" + str(energy)]['n_' + str(i)],
                                                                          var["sumsy_gan" + str(energy)]['n_' + str(i)],
                                                                          var["sumsz_gan" + str(energy)]['n_' + str(i)],
                                                                          var["ecal_gan" + str(energy)]['n_' + str(i)],
                                                                          m, x=x, y=y, z=z)
            for index, a in enumerate(angles):
                indexes = np.where(((var["angle" + str(energy)]) > np.radians(a) - tolerance2) & (
                        (var["angle" + str(energy)]) < np.radians(a) + tolerance2))
                var["events_gan" + str(energy) + "ang_" + str(a)]['n_' + str(i)] = \
                    var["events_gan" + str(energy)]['n_' + str(i)][indexes]
                var["sumsx_gan" + str(energy) + "ang_" + str(a)]['n_' + str(i)] = \
                    var["sumsx_gan" + str(energy)]['n_' + str(i)][indexes]
                var["sumsy_gan" + str(energy) + "ang_" + str(a)]['n_' + str(i)] = \
                    var["sumsy_gan" + str(energy)]['n_' + str(i)][indexes]
                var["sumsz_gan" + str(energy) + "ang_" + str(a)]['n_' + str(i)] = \
                    var["sumsz_gan" + str(energy)]['n_' + str(i)][indexes]
                var["isreal_act" + str(energy) + "ang_" + str(a)]['n_' + str(i)] = \
                    var["isreal_act" + str(energy)]['n_' + str(i)][indexes]
                var["isreal_gan" + str(energy) + "ang_" + str(a)]['n_' + str(i)] = \
                    var["isreal_gan" + str(energy)]['n_' + str(i)][indexes]
                var["aux_act" + str(energy) + "ang_" + str(a)]['n_' + str(i)] = \
                    var["aux_act" + str(energy)]['n_' + str(i)][indexes]
                var["aux_gan" + str(energy) + "ang_" + str(a)]['n_' + str(i)] = \
                    var["aux_gan" + str(energy)]['n_' + str(i)][indexes]
                var["ecal_act" + str(energy) + "ang_" + str(a)]['n_' + str(i)] = \
                    var["ecal_act" + str(energy)]['n_' + str(i)][indexes]
                var["ecal_gan" + str(energy) + "ang_" + str(a)]['n_' + str(i)] = \
                    var["ecal_gan" + str(energy)]['n_' + str(i)][indexes]
                var["angle_act" + str(energy) + "ang_" + str(a)]['n_' + str(i)] = \
                    var["angle_act" + str(energy)]['n_' + str(i)][indexes]
                var["angle_gan" + str(energy) + "ang_" + str(a)]['n_' + str(i)] = \
                    var["angle_gan" + str(energy)]['n_' + str(i)][indexes]
                var["momentX_gan" + str(energy) + "ang_" + str(a)]['n_' + str(i)] = \
                    var["momentX_gan" + str(energy)]['n_' + str(i)][indexes]
                var["momentY_gan" + str(energy) + "ang_" + str(a)]['n_' + str(i)] = \
                    var["momentY_gan" + str(energy)]['n_' + str(i)][indexes]
                var["momentZ_gan" + str(energy) + "ang_" + str(a)]['n_' + str(i)] = \
                    var["momentZ_gan" + str(energy)]['n_' + str(i)][indexes]

                if angloss == 2:
                    var["angle2_act" + str(energy) + "ang_" + str(a)]['n_' + str(i)] = \
                        var["angle2_act" + str(energy)]['n_' + str(i)][indexes]
                    var["angle2_gan" + str(energy) + "ang_" + str(a)]['n_' + str(i)] = \
                        var["angle2_gan" + str(energy)]['n_' + str(i)][indexes]
                if addloss:
                    var["addloss_act" + str(energy) + "ang_" + str(a)]['n_' + str(i)] = \
                        var["addloss_act" + str(energy)]['n_' + str(i)][indexes]
                    var["addloss_gan" + str(energy) + "ang_" + str(a)]['n_' + str(i)] = \
                        var["addloss_gan" + str(energy)]['n_' + str(i)][indexes]
        print(
            'For {} iteration:\nWith Generator weights.....{}\nWith Discriminator weights.....{}'.format(i, gen_weights,
                                                                                                         disc_weights))
    for i in np.arange(len(gweights)):
        ...
        for energy in energies:
            print("{}\t{}\t{:.4f}\t\t{}\t\t\t{:.2f}\t\t{:.4f}\t\t{:.4f}\t\t{:.4f}".format(energy,
                                                                                          var["index" + str(energy)],
                                                                                          np.amax(var[
                                                                                                      "events_gan" + str(
                                                                                                          energy)][
                                                                                                      'n_' + str(i)]),
                                                                                          np.mean(var[
                                                                                                      ...
                                                                                                      'n_' + str(i)],
                                                                                                  axis=0), np.mean(
                    var["events_gan" + str(energy)]['n_' + str(i)]), np.mean(
                    var["momentX_gan" + str(energy)]['n_' + str(i)][:, 1]), np.mean(
                    var["momentY_gan" + str(energy)]['n_' + str(i)][:, 1]), np.mean(
                    var["momentZ_gan" + str(energy)]['n_' + str(i)][:, 1])))
    return var


def perform_calculations_multi(g, d, gweights, dweights, energies, datapath, sortdir, gendirs, discdirs, num_data,
                               num_events, m, scales, thresh, flags, latent, events_per_file=10000, particle='Ele',
    ...
    for gen_weights, disc_weights, scale, i in zip(gweights, dweights, scales, np.arange(len(gweights))):
        ...
        for energy in energies:
            ...
            if read_gen:
                ...
                start = time.time()
                var["events_gan" + str(energy)]['n_' + str(i)] = generate(g, var["index" + str(energy)],
                                                                          [var["energy" + str(energy)] / 100], latent)
                var["events_gan" + str(energy)]['n_' + str(i)] = np.squeeze(
                    var["events_gan" + str(energy)]['n_' + str(i)])
                if save_gen:
                    save_generated(var["events_gan" + str(energy)]['n_' + str(i)], var["energy" + str(energy)], energy,
                                   gendir)
                gen_time = time.time() - start
            ...
            if read_disc:
                var["isreal_act" + str(energy)]['n_' + str(i)], var["aux_act" + str(energy)]['n_' + str(i)], \
                var["ecal_act" + str(energy)]['n_' + str(i)], var["isreal_gan" + str(energy)]['n_' + str(i)], \
                var["aux_gan" + str(energy)]['n_' + str(i)], var["ecal_gan" + str(energy)]['n_' + str(i)] = get_disc(
                    energy, discdir)
            else:
                d.load_weights(disc_weights)
                start = time.time()
                if dformat == 'channels_last':
                    ...
                    var["events_gan" + str(energy)]['n_' + str(i)] = np.expand_dims(
                        var["events_gan" + str(energy)]['n_' + str(i)], axis=-1)
                else:
                    var["events_act" + str(energy)] = np.expand_dims(var["events_act" + str(energy)], axis=1)
                    var["events_gan" + str(energy)]['n_' + str(i)] = np.expand_dims(
                        var["events_gan" + str(energy)]['n_' + str(i)], axis=1)
                ...
                print(len(discout))
                var["isreal_act" + str(energy)]['n_' + str(i)], var["aux_act" + str(energy)]['n_' + str(i)], \
                var["ecal_act" + str(energy)]['n_' + str(i)] = discriminate(d, var["events_act" + str(energy)] * scale)
                var["isreal_gan" + str(energy)]['n_' + str(i)], var["aux_gan" + str(energy)]['n_' + str(i)], \
                var["ecal_gan" + str(energy)]['n_' + str(i)] = discriminate(d, var["events_gan" + str(energy)][
                    'n_' + str(i)])
                disc_time = time.time() - start
                print("Discriminator took {} seconds for {} data and generated events".format(disc_time, var[
                    "index" + str(energy)]))
            ...
            var["isreal_act" + str(energy)]['n_' + str(i)], var["aux_act" + str(energy)]['n_' + str(i)], \
            var["ecal_act" + str(energy)]['n_' + str(i)] = np.squeeze(
                var["isreal_act" + str(energy)]['n_' + str(i)]), np.squeeze(
                var["aux_act" + str(energy)]['n_' + str(i)]), np.squeeze(
                var["ecal_act" + str(energy)]['n_' + str(i)] / scale)

            var["isreal_gan" + str(energy)]['n_' + str(i)], var["aux_gan" + str(energy)]['n_' + str(i)], \
            var["ecal_gan" + str(energy)]['n_' + str(i)] = np.squeeze(
                var["isreal_gan" + str(energy)]['n_' + str(i)]), np.squeeze(
                var["aux_gan" + str(energy)]['n_' + str(i)]), np.squeeze(
                var["ecal_gan" + str(energy)]['n_' + str(i)] / scale)
            var["max_pos_gan" + str(energy)]['n_' + str(i)] = get_max(var["events_gan" + str(energy)]['n_' + str(i)])
            var["sumsx_gan" + str(energy)]['n_' + str(i)], var["sumsy_gan" + str(energy)]['n_' + str(i)], \
            var["sumsz_gan" + str(energy)]['n_' + str(i)] = get_sums(var["events_gan" + str(energy)]['n_' + str(i)])
            var["momentX_gan" + str(energy)]['n_' + str(i)], var["momentY_gan" + str(energy)]['n_' + str(i)], \
